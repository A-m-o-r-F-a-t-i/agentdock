...
选择 workspace、shared 或插件成员时，读取宿主返回的 `file`，执行时使用同一候选的 `skill_ref`。不要在后续步骤按裸名称改选另一来源。独立已安装 Skill 仍由 `skill_package` 负责版本、激活和回滚，插件成员继续由 `plugin_manage` 管理。

managed Skill 执行时，宿主可提供保留变量 `SKILL_DATA_DIR` 作为私有持久数据目录。可移植核心不得把它设为用户必填配置或硬编码宿主路径；shared/workspace 同名候选不继承 managed 环境和数据。安装包禁止符号链接、FIFO 和其他特殊文件。managed 资源读取及命令执行持有读租约，写入、激活和卸载等待该读取结束。

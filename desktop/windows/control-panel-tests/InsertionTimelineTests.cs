...
internal static class InsertionTimelineTests
{
    ...
    internal static void Run(Action<bool, string> check)
    {
        ...
        JsonElement Message(string status, int attempt = 1, int update = 2, string evidence = "") => Json(new {
            insertion_id = "ins_1", conversation_id = "conv_1", task_id = "task_1", text = "你好，我是帅哥\n后续要求",
            status, delivery_attempts = attempt, created_at = start.AddSeconds(1), updated_at = start.AddSeconds(update), expires_at = start.AddMinutes(5),
        ...
        check(message.InsertionDetails.Contains("你好，我是帅哥\n后续要求") && message.InsertionDetails.Contains("call_original") && message.InsertionDetails.Contains("outer_projection"), "message details retain content and actual correlation IDs");
        ...
        check(!ExecutionCallRow.FromInsertion(Message("delivery_unknown",6),start.AddSeconds(6)).CanRedeliverInsertion, "total redelivery cap is visible");

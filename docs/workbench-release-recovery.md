...
　　恢复工作流验证原工作流路径、仓库、SHA、完成状态、构建/验证任务及完整资产组装步骤。1.1.7 完整补缺版及后续版本必须额外核对两个 Windows 原生恢复任务与 ARM64 成品安装任务，总计十二个必须成功的作业，x64 安装及 Linux 包事务还须通过发行报告检查。随后下载该 run 的原始平台资产和报告，以原 SHA 独立检出的源码目录重新执行同一个发布器的完整组装、摘要和报告一致性检查。恢复脚本提交与构建源码提交分别写入 `publication-result.json`，不改动既有发行标签，不重新编译或混用其他 run 的安装包。
...
　　常规发布器继续拒绝修改已发布版本和移动标签。只有用户明确要求替换某个既有版本时，才进入单独的替换流程：先备份原 Release 元数据、标签 SHA 和全部资产，重新以目标版本编译全部平台，通过完整原生门禁和摘要校验后，将指定 Release 转为草稿，按已核对的数字 ID 移除旧资产，并用旧 SHA 约束更新该标签。随后使用已验证构建的发布入口上传完整新资产并设为 Latest。强制移动标签不会触发第二套自动发布。

　　2026-09-25 的版本归属更正由用户明确要求：1.1.7 查漏补缺不另发1.1.8，全部修复和原生验收归回1.1.7。误发1.1.8保留本机可验证备份，撤回其公开发行。该一次性授权不改变其他版本或后续发布的默认保护规则。

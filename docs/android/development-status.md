...
Android 编译、Lint、单元测试及仪器测试只在 GitHub Actions 执行。模拟器覆盖 API 26、33、34、35、37。每个模拟器必须记录真实 API、至少 7 项通过的仪器测试和全部 18 张规定截图，缺失报告或截图会失败。传输测试使用实际 loopback HTTP 加固定响应，不能替代真实 Core/Termux 联调。

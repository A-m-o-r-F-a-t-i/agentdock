...
docker exec "$runtime_container" sh -c '
  test -f "$HOME/.agentdock/skills/.system/.agentdock-system-skills.marker"
  for skill in agentdock-user-guide skill-authoring skill-installation; do
    state="$HOME/.agentdock/skills/.state/$skill.json"
    test "$(jq -r .system "$state")" = "true"
    test -n "$(jq -r .active_version "$state")"
    test -f "$HOME/.agentdock/skills/.system/$skill/SKILL.md"
  done
'

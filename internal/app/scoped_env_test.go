...
func TestSkillEnvironmentActionsDoNotReturnValuesAndExecCommandUsesPriority(t *testing.T) {
	...
	defer runtime.Close()
	installDocumentSkillForTest(t, runtime, "demo-skill", "1.0.0", "Demo Skill.")
...
func TestExecCommandSchemaKeepsExistingSelectorsAndScopedReference(t *testing.T) {
	props := testInputSchema("exec_command")["properties"].(map[string]any)
	for _, field := range []string{"skill_ref", "skill", "skill_env"} {
		if props[field] == nil {
			t.Fatalf("existing or scoped selector missing: %s", field)
		}
	}
}
...
func TestManagedExecInjectsPrivateSkillDataDirAndRejectsOverrides(t *testing.T) {
	runtime := newScopedEnvTestRuntime(t)
	defer runtime.Close()
	installDocumentSkillForTest(t, runtime, "demo-skill", "1.0.0", "Managed data Skill.")
...
func TestSharedSkillWithSameNameDoesNotReceiveManagedEnvironment(t *testing.T) {
	...
	runtime := newScopedEnvTestRuntime(t)
	defer runtime.Close()
	installDocumentSkillForTest(t, runtime, "demo-skill", "1.0.0", "Managed duplicate.")
	...
	if _, err := runtime.Call(t.Context(), "exec_command", map[string]any{"cmd": command, "skill_ref": "skill://shared/demo-skill", "skill_env": "demo-skill", "execution_mode": "sync"}); err == nil {
		t.Fatal("shared exact reference borrowed managed credentials")
	}
...
func TestExecCommandPreservesExistingSkillSelectors(t *testing.T) {
	rt := newScopedEnvTestRuntime(t)
	defer rt.Close()
	root := installDocumentSkillForTest(t, rt, "demo-skill", "1.0.0", "Legacy selector fixture.")
	for _, field := range []string{"skill", "skill_env", "skill_ref"} {
		value := "demo-skill"
		if field == "skill_ref" {
			value = "skill://managed/demo-skill"
		}
		result, err := rt.Call(t.Context(), "exec_command", map[string]any{"cmd": commandNoopForTest(), field: value, "execution_mode": "sync"})
		if err != nil || result["command_ok"] != true {
			t.Fatalf("selector %s failed: %#v %v", field, result, err)
		}
		expected := root
		if field == "skill_env" {
			expected = rt.cfg.AgentDockDefaultDir
		}
		if !sameExistingTestPath(fmt.Sprint(result["workdir"]), expected) {
			t.Fatalf("selector %s changed cwd semantics: %#v", field, result)
		}
	}
}

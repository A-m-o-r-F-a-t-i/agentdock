...
func newWorkspaceContextRuntime(t *testing.T) (*Runtime, string) {
	...
	globalPath := filepath.Join(home, ".agentdock", "AGENTS.md")
	if err := os.MkdirAll(filepath.Dir(globalPath), 0700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(globalPath, []byte("Fixture global rules.\n"), 0600); err != nil {
		t.Fatal(err)
	}
	cfg := config.Config{
		InstructionsFile:    filepath.Join(home, ".agentdock", "AGENTS.md"),
...
func TestWorkspaceContextLoadsConfiguredGlobalNestedRulesAndLocalSkills(t *testing.T) {

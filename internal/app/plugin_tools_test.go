...
func TestPluginComponentsEnterExistingRuntimeAndSkillExecUsesPluginData(t *testing.T) {
	...
	if definition, ok := installed["plugin"].(pluginruntime.Definition); !ok || definition.Name != "demo.plugin" || definition.Version != "1.0.0" {
...
func TestPluginLifecycleKeepsStandaloneMCPAndOwnsMCPEnvironment(t *testing.T) {
	...
	if _, err := rt.Call(context.Background(), "mcp_manage", map[string]any{
		...
		assertToolErrorCode(t, err, "MCP_SERVER_MANAGED_BY_PLUGIN")
	}
	...
	if _, err := rt.Call(context.Background(), "plugin_manage", map[string]any{
		"action": "remove", "name": "demo.plugin",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(envPath); err != nil {
	...
	marker := filepath.Join(dataDir, "user-data.txt")
	if err := os.WriteFile(marker, []byte("keep me"), 0600); err != nil {
		t.Fatal(err)
	}
	if _, err := rt.Call(t.Context(), "plugin_manage", map[string]any{"action": "remove", "name": "demo.plugin", "data_policy": "purge"}); err == nil {
		t.Fatal("unimplemented purge was silently accepted")
	}
	if _, err := rt.Call(t.Context(), "plugin_manage", map[string]any{"action": "remove", "name": "demo.plugin"}); err != nil {
		t.Fatal(err)
	}
	for _, path := range []string{envPath, marker} {
		if _, err := os.Stat(path); err != nil {
			t.Fatalf("default remove lost user data %s: %v", path, err)
		}
	}
...
func TestPluginUpdateOwnershipPreflightPreservesPreviousPackageAndStandaloneMCP(t *testing.T) {
	rt, root := newPluginTestRuntime(t)
	...
	beforeDef := before["plugin"].(pluginruntime.Definition)
	beforePackage, err := pluginruntime.LoadPackage(beforeDef.Path)
	if err != nil {
		t.Fatal(err)
	}
	beforeDigest := beforePackage.PackageDigest
	...
	_, err = rt.Call(context.Background(), "plugin_manage", map[string]any{
		"action": "update", "name": "demo.plugin", "source": sourceV2, "confirmed": true, "confirmed_source_change": true,
	})
	assertToolErrorCode(t, err, "PLUGIN_MEMBER_CONFLICT")
	...
	afterDef := after["plugin"].(pluginruntime.Definition)
	afterPackage, err := pluginruntime.LoadPackage(afterDef.Path)
	if err != nil {
		t.Fatal(err)
	}
	if afterDef.Version != "1.0.0" || afterPackage.PackageDigest != beforeDigest {
...
func TestPluginRemovalPreservesHistoricalMCPEnvironmentAfterUpdate(t *testing.T) {
	rt, root := newPluginTestRuntime(t)
	...
	if _, err := os.Stat(envPath); err != nil {
		t.Fatalf("default removal lost historical MCP environment: %v", err)
	}
}

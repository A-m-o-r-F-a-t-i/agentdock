...
func TestPluginOwnedStdioUsesStableEnvAndRuntimeProvenance(t *testing.T) {
	...
	if err := manager.SetExternalServerProvider(func() (map[string]ServerConfig, error) {
		return map[string]ServerConfig{runtimeName: {
			...
			Args:       []string{"-test.run=TestMCPStdioHelperProcess"},
			PackageEnv: map[string]string{"GO_WANT_MCP_HELPER": "1", "MODE": "package-default"},
			EnvFromEnv: map[string]string{"TOKEN": "DEMO_TOKEN"},
			StorageKey: storageKey, SourceType: "plugin", PluginName: "demo.plugin",
			PluginRoot: pluginRoot, PluginData: pluginData, Enabled: true,
		}}, nil
	}); err != nil {
		t.Fatal(err)
	}
	...
	if got := readEnv("PLUGIN_ROOT"); got != pluginRoot {
		t.Fatalf("fork PLUGIN_ROOT contract changed: %q", got)
	}
	if got := readEnv("PLUGIN_DATA"); got != pluginData {
		t.Fatalf("fork PLUGIN_DATA contract changed: %q", got)
	}

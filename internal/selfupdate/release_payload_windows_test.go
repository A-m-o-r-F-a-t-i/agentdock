...
func TestWindowsReleasePayloadRejectsUnsafeCatalogue(t *testing.T) {
	tests := []struct {
		...
		{name: "traversal", want: "路径", add: func(writer *zip.Writer) { writeReleaseEntry(t, writer, "../escape.exe", "escape") }},
		{name: "case folded duplicate", want: "重复条目", add: func(writer *zip.Writer) { writeReleaseEntry(t, writer, "AGENTDOCK.EXE", "duplicate") }},
		{name: "symlink", want: "不是普通文件", add: func(writer *zip.Writer) {
			header := &zip.FileHeader{Name: "unused-link"}
			header.SetMode(os.ModeSymlink | 0o777)
			entry, err := writer.CreateHeader(header)
			if err != nil { t.Fatal(err) }
			if _, err := entry.Write([]byte("agentdock.exe")); err != nil { t.Fatal(err) }
		}},
	}
...
func makeWindowsReleaseZIP(t *testing.T, add func(*zip.Writer)) []byte {
	...
	for _, file := range files { writeReleaseEntry(t, writer, file[0], file[1]) }
	if add != nil { add(writer) }
...
func writeReleaseEntry(t *testing.T, writer *zip.Writer, name, content string) {
	...
	if err != nil { t.Fatal(err) }
	if _, err := entry.Write([]byte(content)); err != nil { t.Fatal(err) }
}
...
func assertReleaseFile(t *testing.T, path, expected string) {
	...
	if err != nil { t.Fatal(err) }
	if string(data) != expected { t.Fatalf("%s=%q, want %q", path, data, expected) }
}

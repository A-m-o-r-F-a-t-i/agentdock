...
// same-version repair 的 generation 内容重建由引擎拥有：payload 覆盖旧内容，
// committed pointer 保持不变，journal 快照提供旧内容的回滚恢复。
func TestWindowsRestagesOwnedGenerationForSameVersionRepair(t *testing.T) {
	root := t.TempDir()
	payload := filepath.Join(root, "payload")
	if err := os.MkdirAll(payload, 0o755); err != nil {
		t.Fatal(err)
	...
	journal := newJournal(request.InstallRoot, "owned-2")
	staged, err := stageWindowsPayload(request, journal)
	if err != nil {
		t.Fatal(err)
	}
	...
	if string(got) != "core-v2" {
		t.Fatalf("same-version repair did not restage the generation: %q", got)
	}
	if staged.Binary != layout.GenerationCore("v1.0.0") {
		t.Fatalf("staged binary=%s", staged.Binary)
	}
	activeStore, err := updateengine.NewStore(request.InstallRoot)
	if err != nil {
		t.Fatal(err)
	}
	active, err := activeStore.ReadActive()
	if err != nil {
		t.Fatal(err)
	}
	if err != nil {
	...
	if active.State != updateengine.StateCommitted || active.TransactionID != "owned-1" {
		t.Fatalf("repair must keep the committed pointer owned by the source transaction, got state=%s tx=%s", active.State, active.TransactionID)
	}
	if err := journal.Restore(context.Background(), request); err != nil {
		t.Fatal(err)
	}
	restored, err := os.ReadFile(layout.GenerationCore("v1.0.0"))
	if err != nil {
		t.Fatal(err)
	}
	if string(restored) != "core-v1" {
		t.Fatalf("journal restore did not bring back the old generation content: %q", restored)
	}
}

// 没有 payload 的 repair（复用现有 generation）必须仍然 attach，不能凭空重建。
func TestWindowsAttachesOwnedGenerationWithoutPayload(t *testing.T) {
	root := t.TempDir()
	payload := filepath.Join(root, "payload")
	if err := os.MkdirAll(payload, 0o755); err != nil {
		t.Fatal(err)
	...
	if string(got) != "core-v1" {
		t.Fatalf("attach must not touch the owned generation: %q", got)
	}
	if staged.Binary != layout.GenerationCore("v1.0.0") {
...
// desktop-version.txt 是 selfupdate 与 Tray 的版本投影，engine-ready 时由引擎统一写入。
func TestActivateWindowsWritesDesktopVersionFile(t *testing.T) {
	root := t.TempDir()
	payload := filepath.Join(root, "payload")
	if err := os.MkdirAll(payload, 0o755); err != nil {
		t.Fatal(err)
	...
	journal := newJournal(request.InstallRoot, "desktop-version")
	staged, err := stageWindowsPayload(request, journal)
	if err != nil {
		t.Fatal(err)
	}
	...
	data, err := os.ReadFile(filepath.Join(request.InstallRoot, "desktop-version.txt"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.TrimSpace(string(data)) != "v1.0.0" {
		t.Fatalf("desktop-version.txt=%q, want v1.0.0", string(data))
	}
}

...
func activateWindows(ctx context.Context, request Request, staged stagedInstall) (activatedInstall, error) {
	...
	// desktop-version.txt 是 selfupdate 与 Tray 的版本投影。engine-ready 时 payload 版本
	// 已在 preflight 验证过，由引擎统一写入并纳入 journal；脚本只在非 engine-ready 兜底
	// 路径自己写。unknown/dev 版本不能落盘，否则会污染 selfupdate 的版本判断。
	if version := updateengine.NormalizeVersion(request.Version); version != "" && version != "vunknown" && version != "vdev" {
		versionPath := filepath.Join(request.InstallRoot, "desktop-version.txt")
		if err := staged.Journal.Snapshot(versionPath); err != nil {
			return activatedInstall{}, err
		}
		if err := atomicfile.Write(versionPath, []byte(version), 0o644); err != nil {
			return activatedInstall{}, err
		}
	}
...
func stageWindowsPayload(request Request, journal *rollbackJournal) (stagedInstall, error) {
	...
	// 已有 active-version / generation 时 Installer 只附着或做 same-version repair 重建，
	// 禁止再删再写同一目录而不留恢复路径。
	if strings.TrimSpace(request.PayloadDir) == "" || windowsGenerationAlreadyOwned(request) {
		if repairWindowsRestageRequired(request) {
			return repairWindowsGeneration(request, journal, layout)
		}
...
// repairWindowsRestageRequired 判断 same-version repair 是否要由引擎重建 generation 内容。
// attach 只复用旧内容，无法把新 payload 的修复发布进同版本 generation；此时必须重建，
// 但 pointer 保持 committed 不动，事务边界只是 generation 目录内容。
func repairWindowsRestageRequired(request Request) bool {
	if strings.TrimSpace(request.PayloadDir) == "" {
		return false
	}
	target := updateengine.NormalizeVersion(request.Version)
	if target == "" || target == "vunknown" || target == "vdev" {
		return false
	}
	active := windowsCommittedGeneration(request)
	return active != "" && updateengine.NormalizeVersion(active) == target
}

// copyWindowsGenerationPayload 把 payload 里的 immutable generation 内容拷进 staging 目录。
// publish 与 same-version repair 重建共用同一份内容契约。
func copyWindowsGenerationPayload(payload, staging string) error {
	...
	for _, item := range copies {
		if !fileExists(item.src) {
			return fmt.Errorf("payload 缺少 %s", filepath.Base(item.src))
		}
...
// repairWindowsGeneration 用 payload 内容重建与 committed pointer 同版本的 generation。
// 与 publishWindowsGeneration 的差别：pointer 已 committed 且指向同一版本，绝不能把
// committed pointer 改写成 trial；journal Snapshot 记录旧内容，崩溃恢复时把 known-good
// 内容拷回来。目录替换与旧版 PowerShell `.repair-backup` 语义一致，只是回滚证据改由
// engine journal 持有。
func repairWindowsGeneration(request Request, journal *rollbackJournal, layout updateengine.WindowsLayout) (stagedInstall, error) {
	if err := layout.EnsureBase(); err != nil {
		return stagedInstall{}, err
	}

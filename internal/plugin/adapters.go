...
func (m *Store) adaptStagedSource(staged stagedPluginSource, request SourceRequest) (string, Package, func(), error) {
	...
	normalized, err := m.TempPath("adapted")
	if err != nil {
		return "", Package{}, func() {}, err
	}
...
func normalizeOpenAIPlugin(sourceRoot, normalizedRoot string, request SourceRequest, staged stagedPluginSource) (Compatibility, error) {
	...
	raw, err := readSourceJSONObject(manifestPath, maxManifestBytes)
	if err != nil {
...
func readSourceJSONObject(path string, maxBytes int64) (map[string]json.RawMessage, error) {

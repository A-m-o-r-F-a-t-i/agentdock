...
func TestOpenAIAdapterNormalizesCodexPluginIntoP2Model(t *testing.T) {
	...
	result, err := manager.InstallSource(context.Background(), SourceRequest{Type: "local", Ref: root}, false, false)
	if err != nil {
		t.Fatal(err)
	}
	installed, err := manager.Get(result.Name)
	if err != nil {
		t.Fatal(err)
	}
	...
	pkg, err := LoadPackage(installed.Path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClaudeAdapterNormalizesManifestMCPAndPersistentPlaceholders(t *testing.T) {
	...
	installed, err := manager.Get("claude-demo")
	if err != nil {
		t.Fatal(err)
	}
	pkg, err := LoadPackage(installed.Path)
...
func TestGitSourcePinsResolvedCommitWithoutRunningWorktreeHooks(t *testing.T) {
	...
	installed, err := manager.Get("git-demo")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCatalogUnderlyingSourceChangeRequiresExplicitRebind(t *testing.T) {
	...
	if _, err := manager.InstallSource(context.Background(), request, false, false); err != nil {
		t.Fatal(err)
	}

	writeCatalog("two")
	_, err = manager.InstallSource(context.Background(), request, true, false)
	var pluginErr *Error
	if !errors.As(err, &pluginErr) || pluginErr.Code != "PLUGIN_SOURCE_CHANGE_CONFIRMATION_REQUIRED" {
		t.Fatalf("catalog source move error = %#v", err)
	}
	if _, err := manager.InstallSource(context.Background(), request, true, true); err != nil {
		t.Fatal(err)
	}
	installed, err := manager.Get("catalog-demo")
	if err != nil {
		t.Fatal(err)
	}
	if installed.Version != "2.0.0" || installed.Source.ResolvedRef != "plugins/two" {
		t.Fatalf("catalog source rebind = %#v", installed)
	}
}

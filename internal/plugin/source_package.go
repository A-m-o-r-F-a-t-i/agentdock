...
func LoadPackage(root string) (Package, error) {
	...
	if entries, readErr := os.ReadDir(skillRoot); readErr == nil {
		for _, entry := range entries {
			...
			doc, err := skills.LoadPortableSkillDocument(componentRoot)
			if err != nil {
...
func loadManifest(path string) (Manifest, []string, []string, error) {
	manifest, diagnostics, err := readManifest(filepath.Dir(path))
	if err != nil {
		return Manifest{}, nil, nil, pluginError("PLUGIN_MANIFEST_INVALID", "manifest.read", err)
	}
	// Opaque extension namespaces retain the fork's portable semantics. They are
	// not executable components and cannot invalidate otherwise valid packages.
	if manifest.Version == "" {
		manifest.Version = VersionLocal
		diagnostics = append(diagnostics, "plugin.json omits version; local portable source is treated as version=local")
	}
	return manifest, nil, diagnostics, nil
}
